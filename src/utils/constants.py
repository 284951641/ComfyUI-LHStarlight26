...
def get_all_model_paths() -> list:
    ...
    try:
        ...
        # RunningHub private uploads may be mounted as UNET/diffusion/VAE
        # models instead of directly under models/SEEDVR2. Include those
        # registered locations without copying multi-gigabyte files.
        if hasattr(folder_paths, 'folder_names_and_paths'):
            for folder_type in ('diffusion_models', 'unet', 'vae', 'checkpoints'):
                entry = folder_paths.folder_names_and_paths.get(folder_type)
                if entry:
                    all_paths.extend(entry[0])
